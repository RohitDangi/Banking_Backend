...
const sendError = (data, res) => {
    ...
    if (typeof data == 'object' && !data.isBoom) {
        if (data.name == 'MongoError') { // Check Mongo Error
            message = responseMessages.DB_ERROR;
            if (data.code == 11000) {
                if (data.message.split(" ").indexOf('email_1') > -1) {
                    const conflictError = Boom.conflict(responseMessages.EMAIL_ALREADY_EXISTS)
        ...
        } else if(data.hasOwnProperty("sqlMessage")) { // Check My SQL ERROR
